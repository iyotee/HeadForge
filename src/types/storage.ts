// Storage type definitions for Chrome Extension
...
// Application-specific storage interfaces
...
export interface AnalyticsData {
  ...
  exportFormatUsage: { [format: string]: number };
  errorCount: number;
  lastUsed: string;
...
export interface StorageService {
  ...
  // Batch operations
  getMultiple<T>(keys: string[], area?: 'local' | 'sync' | 'managed'): Promise<{ [key: string]: T }>;
  setMultiple<T>(items: { [key: string]: T }, area?: 'local' | 'sync' | 'managed'): Promise<void>;
  ...
  getQuota(area?: 'local' | 'sync' | 'managed'): Promise<{ used: number; available: number; total: number }>;
}
